...
#define list_for_each_entry(pos, head) for (pos = head; pos; pos = pos->next)
#define list_reverse_for_each_entry(pos, head)                                \
  for (pos = head; pos; pos = pos->prev)
...
#define list_goto_first(type, list)                                           \
  do                                                                          \
    {                                                                         \
      type *_pos;                                                             \
      type *_head = list;                                                     \
      list_reverse_for_each_entry (_pos, _head) { list = _pos; }              \
    }                                                                         \
  while (0)
  ...
  union
  {
    ...
    struct
    {
      ...
      union
      {
        discrete_set *discrete_set;
        expr_or_range_list
            *list; /* Union of expression and range definition */
      };
